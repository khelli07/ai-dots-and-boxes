...
class Dots_and_Boxes:
    ...
    # ------------------------------------------------------------------
    def __init__(
        self, config: str, bot1: Optional[Bot] = None, bot2: Optional[Bot] = None
    ):
        self.window = Tk()
    ...
    def load_config(self):
        ...
        f = list(reversed(open("./configs/" + self.config, "r").readlines()))
        ...
        for move in moves:
            player, type_, x, y = move
            self.make_edge(type_, (x, y), player)

        self.player1_starts = is_player
        self.board_status = np.array(array[2][::-1])
        self.row_status = np.array(array[1][::-1])
        self.col_status = np.array(array[0][::-1])
    ...
    def make_edge(self, type, logical_position, player: bool = None):
        if type == "row":
            start_x = (
        ...
        if player is not None:
            player1_turn = True if player else False
        else:
